    ...
    public sealed class RepositorySelectUrn<T> : IVerifyRepository<T>
    {
        ...
        public void Verify(IRepository<T> repository)
        {
            ...
            using (new TransactionScope())
            {
                ...
                if (null == record)
                {
                    throw new UnitTestException(Resources.Repository_Select_Urn_ReturnsNull_UnitTestExceptionMessage);
                }

                if (key != record.Key)
                {
                    throw new UnitTestException(Resources.Repository_Select_Urn_ReturnsIncorrectKey_UnitTestExceptionMessage);
                }

                if (Record.Object.Urn != record.Urn)
                {
                    throw new UnitTestException(Resources.Repository_Select_Urn_ReturnsIncorrectUrn_UnitTestExceptionMessage);
                }
            }
        }
    }
